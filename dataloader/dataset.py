...
class MEDataset(Dataset):
    ...
    def __init__(self, data_info: pd.DataFrame, label_mapping: dict,
                 ...
                 train: bool, EVM: bool, val: bool = False):
        ...
        self.magnet.load_state_dict(torch.load("dataloader/weight/magnet.pt",
                                               map_location=device))
    ...
    def __getitem__(self, idx: int):
        ...
        apex_name = self.data_info.loc[idx, "ApexFrame"]
        ...
        if self.catego == "SAMM":
            ...
            onset_path = f"{self.image_root}/sub{subject.zfill(2)}/{folder}/img{onset_name}.jpg"
            apex_path = f"{self.image_root}/sub{subject.zfill(2)}/{folder}/img{apex_name}.jpg"
        ...
        if not self.EVM:
            ...
            apex_frame = cv2.imread(apex_path)
            images_name = [f"{self.image_root}/sub{subject.zfill(2)}/{folder}/img{idx}.jpg"
                           for idx in range(onset_name, apex_name + 1)]
            # 这里的unit_preprocessing方法在归一化的同时，将numpy格式转换为了torch.Tensor
            # 由于EVM是对numpy格式进行放大，因此模仿着写一个相适应的unit_preprocessing_EVM
            images = [unit_preprocessing_EVM(cv2.imread(image)) for image in images_name]
            if self.train:
                amp_factor = random.choice(MEDataset.AMP_LIST)
            else:
                amp_factor = 2.0
            magnify = magnify_postprocessing_EVM(apex_frame)
            shape_representation = magnify_motion(images, 0.4, 3, amplification=amp_factor, train=True)
            shape_representation = unit_postprocessing_EVM(shape_representation)
            if self.val:
                print(images_name[0], images_name[-1], len(images))
