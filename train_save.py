...
def train_and_evaluate(LOSO: int, epochs: int, criterion: nn.Module, optimizer: torch.optim,
                       ...
                       device: torch.device, model_best_name: str, conf_matrix: torch.Tensor):
    best_train_accuracy = -1
    best_validation_accuracy = -1
    best_validation_f1_score = -1
    conf_matrix_final = torch.zeros_like(conf_matrix)
    ...
    for epoch in range(epochs):
        ...
        ## test
        model.eval()
        conf_matrix_temp = torch.zeros_like(conf_matrix)
        with torch.no_grad():
            test_accuracy = 0.0
            test_f1_score = 0.0
            ...
            for patches, labels in test_loader:
                ...
                conf_matrix_temp = confusion_matrix(output, target, conf_matrix_temp)
            ...
            if test_accuracy > best_validation_accuracy:
                ...
                # conf_matrix_final = torch.Tensor(conf_matrix_temp)
                conf_matrix_final = torch.clone(conf_matrix_temp)
    ...
    return torch.add(conf_matrix, conf_matrix_final), best_validation_accuracy, best_validation_f1_score
...
def LOSO_train(data: pd.DataFrame, sub_column: str, args,
    ...
    for idx in range(len(train_list)):
        ...
        conf_matrix, temp_test_accuracy, temp_f1_score = \
