...
class GCN(nn.Module):
    def __init__(self, adj_matrix, hidden_features: int = 80,
        ...
        # 添加一层LAM
        self.A_L = nn.Parameter(torch.Tensor(9, 9))
        self.A_L.data = adj_matrix
        # self.init_parameters()

    def init_parameters(self):
        std = 1. / math.sqrt(self.A_L1.size(1))
        self.A_L1.data.uniform_(-std, std)
    ...
    def forward(self, x):
        ...
        # 尝试在第一层GCN中添加LAM
        # x = (self.special_matrix + self.A_L1) @ self.graph_weight_one(x)
        # x = F.leaky_relu(x, 0.2)
